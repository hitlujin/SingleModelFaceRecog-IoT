...
class BlazeFace():
    ...
    def detect_save_face(self,image_path,save_path):
        ...
        if len(detections) > 0:
            ...
            if x_min >=0 and y_min >= 0:
                ...
                y_min = int(y_min*ratio)
                if x_max <= img_width and y_max <= img_height:
                    detect_img = img_bgr[y_min:y_max,x_min:x_max]
    ...
    def detect_matimg_result(self,mat_img):
        ...
        start_w,start_h,ratio,d_image=self.preprocess_Image(img_bgr)
        new_image = tf.convert_to_tensor(d_image, np.float32)
        self.interpreter.set_tensor(self.input_idx,new_image)
        self.interpreter.invoke()
        regressions = self.interpreter.get_tensor(self.regressions_idx)
        classifications = self.interpreter.get_tensor(self.classifications_idx)
        ...
        if len(detections) > 0:
            for item,score in zip(detections,scores):
                face_feature = {}
                x_min = int((item[0] * 128 - start_w) * ratio)
                y_min = int((item[1] * 128 - start_h) * ratio)
                if x_min >=0 and y_min >= 0:
                    x_max = int((item[2]*128-start_w)*ratio) 
                    y_max = int((item[3]*128-start_h)*ratio)
                    if x_max <= img_width and y_max <= img_height:
                        face_feature["bbox"] = [x_min,y_min,x_max,y_max]
                        face_feature["face_detect_score"] = float(score)
                        face_feature["landmark"] = []
                        for i in range(6):
                            x = int((item[3+i*2+1] * 128 - start_w) * ratio)
                            y = int((item[3+i*2+2] * 128 - start_h) * ratio)
                            face_feature["landmark"].append((x,y))
                face.append(face_feature)
        return face

if __name__ == "__main__":
    blazeface = BlazeFace('face_detection_front.tflite')
    #blazeface.detect_save_face('smile.jpg','dd.jpg')
    blazeface.detect_show_face('smile.jpg')
    # blazeface.detect_camera_face(0)
